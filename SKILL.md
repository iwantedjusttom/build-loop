...
2. **Make sure it's on the table.** You do **not** move it to a `building` column — Tom organizes the board. Just guarantee the issue is on the board (idempotent if design-queue already added it): `bash /c/Users/iwant/.claude/skills/board-mechanic/board-status.sh <repo> #N` (see *On the table, never moved* below).
...
7. **Finish — open the PR and put it on the table.** *(Reached only on a gate PASS.)* Three actions, in order:
   ...
   - **(c) Put the PR on the table.** Add the PR to the board so it's never missing — **add-only, no column** (Tom moves it where he wants):
     ```
     bash /c/Users/iwant/.claude/skills/board-mechanic/board-status.sh <repo> <pr#>
     ```
...
```
bash /c/Users/iwant/.claude/skills/board-mechanic/board-status.sh <repo> <#>
```

- This is **add-only** — it guarantees the item is on the cross-repo Mission Control board (Project #1) and **never sets a label or slides a card between columns.** Tom organizes the columns himself.
...
- `Closed` is still automatic — the board's built-in "item closed → Closed" workflow fires when the PR merges. You never close the issue yourself.
