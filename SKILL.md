...
- **Status is a board column:** Ready → Building → In-review. A **closed** issue is shipped. Status lives in the Mission Control board columns Tom organizes by hand — **not** in issue labels.
...
2. **Make sure it's on the table.** You do **not** move it to the Building column — Tom organizes the board. Just guarantee the issue is on the board (idempotent if design-queue already added it): `bash /c/Users/iwant/.claude/skills/board-mechanic/board-status.sh <repo> #N` (see *On the table, never moved* below).
